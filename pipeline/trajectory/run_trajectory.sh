...
iter=1
task=alfworld # alfworld | webshop | intercode_sql
...
do
  ...
  log_file="nohup/${task}/${task}_traj_1.5b_iter_${iter}_${part_idx}.log"

  echo "Starting process with part_idx=${part_idx}, port=${current_port}, log_file=${log_file}"
  nohup python pipeline/trajectory/generate_trajectory.py \
    --model 1.5b \
    --iter ${iter} \
    --exp_config ${task} \
...
echo "To check running processes, you can use: ps aux | grep generate_trajectory.py"