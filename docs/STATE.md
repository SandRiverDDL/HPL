...
- WebShop 原始依赖非常旧，固定了旧 `torch/transformers`；当前没有照单安装，否则会破坏 LLaMA-Factory。
- `pyserini` 必须使用与 Java 11 兼容的版本；当前固定为 `pyserini==0.17.0`。
- WebShop 运行依赖 `JAVA_HOME`、`LD_LIBRARY_PATH`、`PYTHONPATH`，不要只激活 `.venv` 就直接跑。
...
- BEACON 当前支持两种 RL 初始化口径：整模型直接传 `actor_rollout_ref.model.path=/path/to/full_hf_model`；LoRA 冷启动传 `actor_rollout_ref.model.path=/path/to/base_model actor_rollout_ref.model.lora_adapter_path=/path/to/lora_adapter`。若同时手写 `lora_rank` 且与 adapter 不一致，会直接报错。
- BEACON LoRA adapter 加载只做过静态编译验证和 HPL adapter config 检查，尚未跑真实 WebShop RL smoke；首次运行建议小 batch、低 `trainer.total_epochs`、低 `data.train_batch_size` 验证。
- BEACON WebShop 本地 patch 支持 `env.webshop.train_start_idx`、`env.webshop.val_end_idx`、`env.webshop.exclude_goal_indices_path`；HPL 启动脚本默认 full human goals，并排除 `data/webshop/test_indices.json`，避免 RL 训练采样污染 HPL test split。
- `scripts/beacon/run_webshop_lora_rl.sh` 是 HPL 控制 BEACON 的 WebShop LoRA RL 编排入口；默认 GPU `1,2,4`、`max_prompt_length=4096`、`max_response_length=512`、`env.rollout.n=4`、`ppo_mini_batch_size=12`、`ppo_micro_batch_size_per_gpu=2`、`lr=1e-5`、关闭 KL loss、`trainer.logger=['console']`。
