...
- BEACON 没有独立轻量 WebShop eval runner；`trainer.val_only=True` 仍会启动 Ray/vLLM/PPO trainer 体系。三组模型横向评测优先走 HPL `evaluation.py`。但 HPL small/synthetic 评测必须先对齐小索引：`num_products=1000` 会使用 `envs/webshop/search_engine/indexes_1k`，当前 config/软链还未完成该修正。

## 下一步

- 若要跑真实 SFT：先准备 HPL 论文所需 expert trajectory 数据，并注册到 LLaMA-Factory dataset registry。
- 若要跑 BEACON small/synthetic 横向评测：先修正 `configs/task/webshop_small_synth_*.json` 的 `num_products=1000` 和 `envs/webshop/search_engine/indexes_1k` 小索引软链，再用 HPL `evaluation.py` 评测 base、SFT LoRA、RL LoRA。
